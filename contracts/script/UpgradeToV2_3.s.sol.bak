...
/**
 * @title UpgradeToV2_3
 * @notice Upgrade script for On-Chain Journal V2.2 → V2.3
 * @dev V2.3 adds ENS name truncation for long names
 *
 * Usage:
 *   forge script script/UpgradeToV2_3.s.sol:UpgradeToV2_3 --rpc-url base_sepolia --broadcast --verify
 *
 * V2.3 Changes:
 * - Added _truncateEnsName() function to handle long ENS names
 * - ENS names longer than 23 chars (25 with checkmark) are truncated with "..."
 * - Fixes ".eti" display issue for "remidot.eth"
 * - Version bumped to 2.3.0
 */
contract UpgradeToV2_3 is Script {
    function run() external {
        uint256 deployerPrivateKey = vm.envUint("PRIVATE_KEY");
        address deployer = vm.addr(deployerPrivateKey);

        console.log("========================================");
        console.log("Upgrading On-Chain Journal to V2.3.0");
        console.log("========================================");
        ...
        console.log("\n1. Deploying new implementation (V2.3)...");
        ...
        require(
            keccak256(bytes(journal.version())) == keccak256(bytes("2.3.0")),
