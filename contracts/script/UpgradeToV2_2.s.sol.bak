...
/**
 * @title UpgradeToV2_2
 * @notice Upgrade script for On-Chain Journal V2.1 → V2.2
 * @dev V2.2 adds updateChainName admin function
 *
 * Usage:
 *   forge script script/UpgradeToV2_2.s.sol:UpgradeToV2_2 --rpc-url base_sepolia --broadcast --verify
 *
 * V2.2 Changes:
 * - Added updateChainName() admin function to fix "Unknown-808813" on Bob
 * - Version bumped to 2.2.0
 */
contract UpgradeToV2_2 is Script {
    function run() external {
        uint256 deployerPrivateKey = vm.envUint("PRIVATE_KEY");
        address deployer = vm.addr(deployerPrivateKey);

        console.log("========================================");
        console.log("Upgrading On-Chain Journal to V2.2.0");
        console.log("========================================");
        ...
        console.log("\n1. Deploying new implementation (V2.2)...");
        ...
        // 3. Fix Bob chain name if on Bob testnet
        if (block.chainid == 808813 || block.chainid == 111) {
            console.log("\n3. Fixing Bob chain name...");
            journal.updateChainName("Bob");
            console.log("Chain name updated to: Bob");
        }
        ...
        require(
            keccak256(bytes(journal.version())) == keccak256(bytes("2.2.0")),
